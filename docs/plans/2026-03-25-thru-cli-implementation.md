...
### Phase 1.2 改进 (2026-03-25) ✅ 完成

| 改进项 | 描述 |
|--------|------|
| send 显示密码提示 | 将 `.output()` 改为 `.status()`，保留 SCP 原生交互 |
| 自动创建目标目录 | 发送前用 `ssh mkdir -p` 创建 `~/storage/downloads/Thru/` |
| .gitignore | 添加 Rust 项目标准忽略规则 |
| 清理 target 目录 | 从 git 版本控制中移除 target/ (1830 个文件) |

### Git 提交记录

```
63e3791 feat: 发送文件前自动创建目标目录
ab88b58 fix: 修复send命令无法显示密码提示的问题, 添加.gitignore
ca54a76 ？？ 怎么有东西没有提交上去 赶紧补一补
21e5df7 docs: 更新项目完成状态
